...
> **Amended 2026-08-02, ratified by owner decision ([ADR-0009](../adr/ADR-0009-deploy-node-server-to-render.md)):**
...
| **M3** | The Node server deployed to Render (ADR-0009) | `/healthz` answers 200 over TLS from the public Render URL, from the bundle CI built |
...
M1 → M2 → M3 are sequential. M3's deploy may start during M2 (it needs no product code) but must not
...
| M1.7 | CI | GitHub Actions, nine named jobs (below). Required on `main`. One server build — the Node bundle Render runs (ADR-0009). |
...
**The nine CI jobs, named:** `typecheck`, `lint`, `lint:boundaries`, `test:unit`, `test:integration`,
`test:security`, `build:web`, `build:server:node`, `secret-scan`.

> *Amended 2026-08-02, ratified by owner decision (ADR-0009).* This was a ten-job list including
> `build:server:cloudflare`. With one deployment target that job builds an artifact nobody deploys, so
> it is deleted rather than kept green. Nine is the number branch protection requires (M1b.5).
...
| M1.7 | GitHub Actions: install, typecheck, lint, boundaries, `build:web`, `build:server:node`, unit, integration (the `build:server:cloudflare` step M1a shipped was removed by ADR-0009) | `test:security`, `secret-scan`, the nine *named* jobs, branch protection |
...
| M1b.3 | M1.6 `tests/security/` B1–B4 + B12 secondary + `b-rows.manifest.json`. **Delivered:** B1/B3/B4 and the manifest gate; then **B2** (M1-AC6) against a real PostgREST — see [ADR-0010](../adr/ADR-0010-supabase-rest-security-harness.md) for the harness decision and its rejected alternatives. **Reassigned, not owed here:** B12's secondary rule is the `no-sql-outside-persistence` boundary rule, which ships in **M1b.9** with the code it binds to; the manifest records B12 as `pending until M2`. | M2's **first viewer-scoped query** |
...
| M1b.5 | M1.7 remainder: nine named CI jobs + branch protection ([#4](https://github.com/drewdrewthis/playa-post/issues/4)) | **any M2 merge** — this is the gate that makes every other row enforceable |
...
- **M1-AC9 (CI)** A PR runs all nine jobs by the names listed above; a PR with a deliberately failing
  `lint:boundaries` cannot be merged. The server build succeeds. *Evidence: PR checks screenshot listing
  the nine job names + a screenshot of the blocked merge button.*
  *(Amended 2026-08-02, ratified by owner decision ADR-0009: ten jobs → nine, and "both server builds"
  → the one Node build.)*
...
five live boundary violations fails the build, proven by its fixture; and the server bundle builds.
M1a explicitly does **not** satisfy M1-AC3 through M1-AC14 — those are M1b, gated per the table
above. *(As shipped, M1a built two server bundles; ADR-0009 removed the second.)*
...
## M3 — The server deployed to Render

> **Rewritten 2026-08-02, ratified by owner decision (ADR-0009).** M3 was a 5-working-day Cloudflare
> runtime spike (criteria S1–S10 plus S3a) whose only product was a verdict on ADR-0001. The owner
> supplied the verdict — the backend is the Node build on Render — so the spike is retired along with
> its work items M3.1–M3.10 and its acceptance criteria. What remains is the deploy itself. Two things
> from the old M3 are carried forward rather than dropped: **S3a**, the deployed-connection-identity
> check, which is a privilege assertion (ADR-0002) and was never really about the runtime; and the
> **§18 no-custom-infrastructure sign-off**, which applies to any milestone that adds dependencies.

**Entry criteria:** M2 in progress or complete; a Supabase project the deployed process can reach.

**Goal:** the M2 server, built by CI, running on Render at a public URL — the first time the system
exists outside a developer's machine. Scope lives in `docs/adr/ADR-0009-deploy-node-server-to-render.md`
and in `render.yaml`. This is a deploy, not an investigation: there is no timebox, no GO/NO-GO, and no
throwaway code.
...
- **M3-AC1 (it is live)** `curl -sS https://<render-url>/healthz` returns HTTP 200 with
  `{"status":"ok"}` over TLS, served by the bundle a CI run built — not a local build uploaded by hand.
  *Evidence: quoted curl transcript including the status line, beside the Render deploy log showing the
  commit SHA it built.*
- **M3-AC2 (the blueprint is the source of truth)** The running service's plan, region, health check
  path, and start command match `render.yaml`, and the service was created from it rather than
  hand-configured. *Evidence: the blueprint-launch output or the service's "Blueprint" linkage in the
  dashboard, quoted or screenshotted, beside the committed file.*
- **M3-AC3 (S3a — deployed connection identity)** The deployed process reports `current_user =
  session_user = 'app_rw'` with `rolsuper = false` and `rolbypassrls = false`. *Evidence: quoted query
  output captured from the deployed process, not from a local psql session.* Unchanged from the retired
  M3-AC8 — connecting as `postgres` to get a deploy green leaves every other check passing while the
  authorization model sits inert (risk R13).
- **M3-AC4 (the slice works deployed)** The M2 e2e suite passes against the deployed URL.
  *Evidence: the Playwright summary line with the deployed base URL visible in the run config.*
- **M3-AC5 (cold start is measured, not assumed)** The free plan's spin-down behaviour is exercised
  deliberately: after ≥ 20 minutes idle, the first request's total time is recorded, and so is a warm
...
**Entry criteria:** M2 exit signal met; M3 exit signal met (the service is live on Render).
...
|---|---|---|---|---|
| R1 | ~~Supabase pooler + Kysely transactions unreliable in `workerd`~~ **Closed 2026-08-02 by ADR-0009** — the runtime is Node, so `workerd`'s constraints do not apply. Replaced by: **free-plan cold start after idle spin-down degrades first-request latency** | Low | Measured, not assumed (M3-AC5); upgrade to a paid instance is a `plan:` line in `render.yaml` with no code change | No |
...
| R9 | ~~The M3 timebox slips and blocks M4~~ **Closed 2026-08-02 by ADR-0009** — there is no spike to time-box; M3 is a deploy | — | — | No |
...
| R13 | The deployed API connects as `postgres` to unblock a deploy, leaving every Testcontainers check green while the model is inert | High | S3a, carried out of the retired spike into M3-AC3; B18/M4-AC4/M6-AC4 gate rollout | No |
...
| 7 | GitHub Actions CI | Nine named jobs: `typecheck`, `lint`, `lint:boundaries`, `test:unit`, `test:integration`, `test:security`, `build:web`, `build:server:node`, `secret-scan`. Required on `main`. `lint:boundaries` is the §22 fitness function (ADR-0009 retired the second server build). | feature |
...
| 25 | Outbox drainer entrypoint (Node poller) | `FOR UPDATE SKIP LOCKED` claiming, exponential backoff `least(15 min, 5s * attempts^2)`, dead-lettering after 8 attempts, `consumer_receipts` for idempotent consumers (ADR-0006). Two concurrent drainers must claim disjoint sets. Runs in-process on the Node server (ADR-0009); no cron variant. | adr |
