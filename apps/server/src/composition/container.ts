...
export function buildAppContainer(configuration: Configuration): AppContainer {
  ...
  // Sync is built last because its registries are adapters over everything above. It is
  // the one module whose dependencies are other modules' use cases rather than the
  // database — which is why the registries are assembled here and never inside it.
