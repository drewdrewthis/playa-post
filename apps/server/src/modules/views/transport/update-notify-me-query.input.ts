 ...
 * `ownerId` and no query identifier: `app.notify_me_queries` is keyed on `owner_id`
 * (ADR-0007:79), so the caller's own resolved `Actor` is the address, and a field
 * naming somebody else's query is not merely forbidden by ADR-0002:180-181 — there is
 * nowhere for it to point. `tests/fitness/viewer-id-provenance.fitness.test.ts` walks
 * this router to prove the field never appears.
 *
