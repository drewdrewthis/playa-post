...
| `integration/` | `notify-me-queries-schema-migration.integration.test.ts` | `app.notify_me_queries`' catalog shape — RLS, ownership, grants, and the one-query-per-owner key `unique (owner_id)` with `id` staying the primary key for outbox aggregate routing (#208, ADR-0019). Plus that an existing untied query survives `20260813214946_remove_saved_views.sql` intact |
