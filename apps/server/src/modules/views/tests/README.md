...
| `integration/` | `saved-view.integration.test.ts` | Saved-views CRUD (issue #45), the viewer-scoping M5-AC16 asks for, and decision D16 — `notify-me.feature`'s "A second saved view can be notified on without switching the first off", "Switching one saved view's notifications off leaves the others on", and "Switching on more notifications than the per-person cap is refused" (#172 AC1/AC2/AC4, ADR-0016). The cap's *scope* is pinned here too, with no scenario of its own: someone at the cap can still write their untied query through `views.notifyMe.update`, because the cap counts bells and the unique key already holds that row at one |
