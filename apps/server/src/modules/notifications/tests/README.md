...
| `integration/` | `notification-optouts-schema-migration.integration.test.ts` | `app.notification_optouts`' catalog shape (issue #209, ADR-0020) — RLS, ownership, grants, the composite primary key on `(owner_id, kind)` that makes "off twice is one row" a constraint, and the CHECK that keeps `kind` inside the contract union |
| `integration/` | `notification-settings.integration.test.ts` | **none** — `notifications.settings.get`/`.update` (issue #209) and the delivery they govern: all-on by default, a converging flip, a queryless connection matched by default (ADR-0020 D1), a `bulletins` opt-out removing someone the graph would serve, and a `note` opt-out meaning no receipt (D4) |
...
| `unit/` | `evaluate-notify-me-multiple-queries.unit.test.ts` | `notify-me.feature` › "A bulletin matching two of one viewer's queries is matched once" — the feature's one `@unit` scenario, tagged that way because the claim settles at evaluation and needs no database — one match per person however many directory rows match (defense in depth over `unique (owner_id)`, #208/ADR-0019), the scan stopping at a person's first match, people kept apart, and the author skipped before any read. Since #209/ADR-0020 the same file also pins the default-on arithmetic: a queryless eligible recipient matches outright, a stored query only narrows, and a person absent from the eligibility read costs and produces nothing. The authorized-read half stays in `notify-me-push.integration.test.ts`: a fake here would prove nothing about `app.visible_bulletins` |
